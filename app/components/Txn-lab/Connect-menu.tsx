...
import { CheckCircle2, Copy, MinusCircle, PlusCircle, Wallet, HandCoins } from "lucide-react";
...
function WalletBalance({ total, available, isMobile = false }: WalletBalanceProps) {
  if (isMobile) {
    return (
      <div className="md:hidden w-full flex flex-row gap-x-6 my-6 bg-slate-100 p-4 rounded-lg">
        <div className="flex flex-row items-center gap-x-3">
          ...
          <div className="flex flex-row items-center">
            <Image className="mr-2" src="/logos/algorandLogo.svg" alt="Algorand logo" width={15} height={15} />
...
export default function ConnectMenu() {
  ...
  const formatBalance = (balance: number) => (balance / Math.pow(10, 6)).toFixed(3);

  return (
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="w-fit border-slate-200 bg-white hover:bg-slate-100" id="connectWallet">
      ...
      <DropdownMenuContent className="w-screen sm:max-w-[screen] md:w-[560px] p-4 rounded-xl">
          ...
          <div className="space-y-4">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              ...
              {isLoading ? (
                ...
                algoBalance && (
                  <>
                    <WalletBalance total={formatBalance(algoBalance[0])} available={formatBalance(algoBalance[0] - algoBalance[1])} />
                    <WalletBalance
                      total={formatBalance(algoBalance[0])}
                      available={formatBalance(algoBalance[0] - algoBalance[1])}
                      isMobile
                    />
                  </>
                )
              ...
              <>
                <div className="flex justify-end">
                  <Button asChild variant="outline" size="sm">
                    <Link href="https://bank.testnet.algorand.network/" target="_blank" className="flex items-center gap-2">
        ...
        <div className="space-y-2 pt-2">
          {wallets?.map((wallet) => (
            <div key={wallet.id} className="py-4 first:pt-0 last:pb-0 border-b border-slate-100 last:border-0">
              {wallet.isActive && wallet.accounts.length ? (
                <div className="flex flex-col md:flex-col gap-4">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium mb-2">{wallet.metadata.name}</h4>
                  ...
                  <Button className="w-full md:w-auto" variant="secondary" onClick={wallet.disconnect} disabled={!wallet.isConnected}>
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  {!wallet.isConnected ? (
                    ...
                    <Button variant="secondary" onClick={wallet.setActive} disabled={!wallet.isConnected || wallet.isActive}>
