...
export function getBackgroundColor(classType: string, hex: boolean = false): string {
...
export function getHeaderBackgroundColor(classType: string, hex: boolean = false): string {
  switch (classType?.toLowerCase()) {
    case "common":
...
export async function checkAlgoBalance(address: string) {
  const { account } = await algorandClient.client.indexer.lookupAccountByID(address).do();
...
export function copyAddress(address: string) {
  window.navigator.clipboard.writeText(address).then(
    () => {
      toast({ variant: "success", title: "Success", description: "Copied wallet address to clipboard!" });
    },
    () => {
      toast({ variant: "error", title: "There was an error", description: "Failed to copy wallet address to clipboard" });
    }
  );
}
export const USDC = {
...
export const isOptedIn = async (address: string, assetId: number) => {
  try {
    const { assets } = await algorandClient.client.indexer.lookupAccountAssets(address).assetId(assetId).do();
...
export const toRawValue = (value: number | undefined, decimals: number): number => {
...
export const calculatePricePerToken = (targetRaise: number | undefined, tokensForSale: number | undefined): number => {
...
export const checkAssetBalance = async (address: string, assetId: number, assetDecimals: number) => {
  try {
    const { balance } = await algorandClient.asset.getAccountInformation(address, BigInt(assetId));
